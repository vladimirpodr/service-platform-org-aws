...
def get_assume_role_credentials(assume_role_arn):
    sts_client = boto3.client('sts')
    try:
        assume_role_response = sts_client.assume_role(RoleArn=assume_role_arn,
...
def evaluate_compliance(event, context):
    ...
    iam_client = get_client('iam', event["accountId"])
    config_client = get_client('config', event["accountId"])
